		...
		public void Initialize(IncrementalGeneratorInitializationContext context)
		{
			...
			context.RegisterSourceOutput(candidates, static (spc, list) =>
			{
				foreach (var (classSymbol, ns) in list)
				{
					...
					sb.AppendLine("using Dreamine.MVVM.Extensions;");
					...
					sb.AppendLine();
					sb.AppendLine("				Application.Current.Dispatcher.InvokeAsync(() =>");
					sb.AppendLine("				{");
					sb.AppendLine("					RegisterAfter(); ");
					sb.AppendLine("				});");			
					sb.AppendLine("        }");
					sb.AppendLine();
					sb.AppendLine("			private static void ForceLoadAssembly(string dllName)");
					sb.AppendLine("			{");
					sb.AppendLine("				var exePath = AppDomain.CurrentDomain.BaseDirectory;");
					sb.AppendLine("				var dllPath = Path.Combine(exePath, dllName);");
					sb.AppendLine("");
					sb.AppendLine("				if (File.Exists(dllPath))");
					sb.AppendLine("				{");
					sb.AppendLine("					var asm = Assembly.LoadFrom(dllPath);");					
					sb.AppendLine("				}");				
					sb.AppendLine("			}");				
					sb.AppendLine();

...
- RelayCommand 스타일 커맨드 생성 지원
- DreamineCommand 기반 forwarding command 생성 지원
...
`[RelayCommand]`가 적용된 메서드를 기준으로 `ICommand` 프로퍼티를 생성합니다.
...
`[DreamineCommand]`가 적용된 메서드를 기준으로 forwarding command와 partial method 구현을 생성합니다.

#### 현재 역할

- `ICommand` 프로퍼티 생성
- `TargetMethod` 호출 코드 생성
- `BindTo`가 있으면 반환값을 지정 프로퍼티에 대입
- 메서드 본문이 비어 있을 때 forwarding body 생성
- 외부 `RelayCommand` 타입에 직접 의존하지 않도록 생성 파일 내부에 전용 `ICommand` 구현을 포함하는 방향 사용
...
- 대상 메서드는 **partial** 이어야 함
- containing type은 **partial** 이어야 함
- 대상 메서드는 **parameterless void** 여야 함
- `TargetMethod`는 비어 있으면 안 됨
- 생성될 command property 이름이 기존 멤버와 충돌하면 생성하지 않음
...
- `RelayCommand`, `DreamineCommand` → method 기반 command 생성

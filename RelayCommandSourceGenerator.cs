...
{
    /// <summary>
    /// <c>RelayCommandAttribute</c>가 적용된 메서드를 기반으로
    /// 커맨드 프로퍼티를 생성하는 증분 생성기입니다.
    /// </summary>
    [Generator]
    public sealed class RelayCommandSourceGenerator : IIncrementalGenerator
    {
        private const string RelayCommandAttributeMetadataName = "Dreamine.MVVM.Attributes.RelayCommandAttribute";
        ...
        private static readonly DiagnosticDescriptor ContainingTypeMustBePartialDescriptor = new(
            ...
            messageFormat: "Containing type '{0}' for [RelayCommand] method must be declared partial",
        ...
        private static readonly DiagnosticDescriptor MethodMustBeParameterlessVoidDescriptor = new(
            id: "DMRLY002",
            title: "RelayCommand method must be parameterless void",
            messageFormat: "Method '{0}' marked with [RelayCommand] must be a parameterless void method",
            category: "Dreamine.MVVM.Generators",
            defaultSeverity: DiagnosticSeverity.Error,
            isEnabledByDefault: true);
        ...
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            IncrementalValueProvider<INamedTypeSymbol?> attributeSymbolProvider =
                context.CompilationProvider.Select(
                    static (compilation, _) => compilation.GetTypeByMetadataName(RelayCommandAttributeMetadataName));

            IncrementalValueProvider<ImmutableArray<RelayCommandCandidateModel>> candidateProvider =
        ...
        /// <param name="attributeSymbol"><c>RelayCommandAttribute</c> 심볼입니다.</param>
        /// <returns>유효한 후보이면 모델을 반환하고, 아니면 <see langword="null"/>을 반환합니다.</returns>
        private static RelayCommandCandidateModel? TryCreateCandidate(
            ...
            INamedTypeSymbol? attributeSymbol)
        {
            ...
            string? commandNameOverride = GetConstructorStringArgument(attributeData, 0);
            string commandPropertyName = BuildCommandPropertyName(methodSymbol.Name, commandNameOverride);

            return new RelayCommandCandidateModel(
                methodSyntax,
                methodSymbol,
                commandPropertyName);
        }
        ...
        private static void Emit(
            ...
            ImmutableArray<RelayCommandCandidateModel> candidates)
        {
            ...
            foreach (RelayCommandCandidateModel candidate in candidates)
            {
        ...
        private static List<Diagnostic> ValidateCandidate(RelayCommandCandidateModel candidate)
        {
        ...
        private static string BuildFileName(RelayCommandCandidateModel candidate)
        {
            ...
            return namespaceName + "_" + typeName + "_" + methodName + "_RelayCommand.g.cs";
        }
        ...
        private static string BuildSource(RelayCommandCandidateModel candidate)
        {
            string namespaceName = candidate.MethodSymbol.ContainingNamespace.IsGlobalNamespace
                ? string.Empty
            ...
            string helperTypeName = "__DreamineGeneratedRelayCommand_" + candidate.MethodSymbol.Name;
        ...
        /// </summary>
        private sealed class RelayCommandCandidateModel
        {
            /// <summary>
            /// <see cref="RelayCommandCandidateModel"/> 클래스의 새 인스턴스를 초기화합니다.
            /// </summary>
            ...
            public RelayCommandCandidateModel(
